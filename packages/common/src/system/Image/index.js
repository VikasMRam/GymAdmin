...
const Image = ({
  ...
  const [failed, setFalied] = React.useState(false);


  const failedLoadImageHandler = () => {
    setFalied(true);
    if (src) {
      setFalied(true);
      onLoadFailed();
  ...
  const sizesProp = loading === 'lazy' ? 'data-sizes' : 'sizes';
  const srcSetProp = loading === 'lazy' ? 'data-srcset' : 'srcSet';
  const getSizes = loading === 'lazy' ? 'auto' : makeSizes(sizes);
  ...
  const picture = failed
    ? (
      ...
      <picture>
        ...
        <img
          key={`${src}_${path}`}
          // loading={loading} {/* disabling this will preload the next images on the slider on chorme */}
          alt={alt || getAlt(path)}
          className={imgClassName}
          data-sizes="auto"
          onError={failedLoadImageHandler}
          {...imageProps}
        />
      </picture>
    );
