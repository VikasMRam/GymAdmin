      ...
      <Wrapper className={className}>
        ...
        <TopWrapper>
          {community.startingRate ? (
            <Rate palette={palette || (inverted ? 'white' : 'secondary')} variation={inverted ? 'base' : 'darker-30'} weight="medium">
              {`${community.estimated ? 'Estimated ' : ''}${communityStartingRate}/month`}
            </Rate>
          ) : null }
          <CommunityRating rating={reviewsValue} numReviews={numReviews} palette={inverted ? 'white' : 'secondary'} variation={inverted ? 'base' : 'darker-30'} numReviewsPalette={inverted ? 'white' : 'slate'} size={reviewsValue > 0 ? 'caption' : 'tiny'} />
        </TopWrapper>
        {address && (
          <IconTextWrapper>
