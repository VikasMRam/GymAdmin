...
describe('resource reducer', () => {
  ...
  describe('RESOURCE_LIST_READ_SUCCESS', () => {
    it('sets list in the initial state', () => {
      expect(reducer(
        initialState,
        action(actions.RESOURCE_LIST_READ_SUCCESS, items([1, 2, 3]))
    ...
    it('overrides list in an existing state', () => {
      expect(reducer(
        state({ list: [1, 2, 3] }),
        action(actions.RESOURCE_LIST_READ_SUCCESS, items([3, 2, 1]))
  ...
  describe('RESOURCE_DETAIL_READ_SUCCESS', () => {
    it('sets detail in the initial state', () => {
      expect(reducer(initialState, action(actions.RESOURCE_DETAIL_READ_SUCCESS, items([1])))).toEqual(state({ detail: 1 }));
    });

    it('overrides detail in an existing state', () => {
      expect(reducer(
        state({ detail: 1 }),
        action(actions.RESOURCE_DETAIL_READ_SUCCESS, items([2]))
