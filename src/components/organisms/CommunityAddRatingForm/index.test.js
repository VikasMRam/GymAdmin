...
const wrap = (props = {}) => shallow(<CommunityAddRatingForm handleSubmit={handleSubmit} communityName={communityName} {...props} />);

describe('CommunityAddRatingForm', () => {
  it('render name and email when user is not passed', () => {
    const wrapper = wrap({ });
    expect(wrapper.find('Heading').html()).toContain(communityName);
