...
const CommunityFilterBar = ({ searchParams, onParamsRemove }) => {
  ...
  return (
    <SectionWrapper>
      {size && <StyledIButton right={true} icon={'close'} onClick={getEvtHandler(['size'], onParamsRemove)}>
        Size
      </StyledIButton>
      }
      {budget && <StyledIButton right={true} icon={'close'} onClick={getEvtHandler(['budget'],onParamsRemove)}>
        Budget
      </StyledIButton>}
      {filtersApplied.length > 0 && <StyledButton onClick={getEvtHandler(filtersApplied,onParamsRemove)} transparent>
        Clear All Filters
      </StyledButton>}
