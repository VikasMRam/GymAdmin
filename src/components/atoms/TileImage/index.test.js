...
const wrap = (props = {}) => shallow(<TileImage {...props} />).dive();
...
describe('TileImage', () => {
  it('renders with default tileSize', () => {
    ...
    expect(wrapper.instance().props.tileSize).toBe(TileImage.defaultProps.tileSize);
  });

  it('renders with passed tileSize', () => {
    const wrapper = wrap({ src, tileSize });
    expect(wrapper.find('img')).toHaveLength(1);
    expect(wrapper.instance().props.tileSize).toBe(tileSize);
  });
